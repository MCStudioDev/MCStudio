...
## Diet, cuisine, and free-tier alignment (2026-05-14 pass)

Following a production diagnostic against two real accounts (`bekhitc12@gmail.com` premium, `christine.maher.fathi@gmail.com` free), the recipe generation and weekly meal-plan paths were tightened on seven axes:

1. **Server-side diet/allergen gate.** New `src/lib/dietEnforcement.ts` exposes ingredient-based forbidden-pattern lists per diet (vegan, vegetarian, dairy-free, gluten-free) and per allergen (dairy, gluten, eggs, fish, shellfish, nuts, soy, sesame), in both English and Arabic. `POST /api/generate-recipes` filters every AI and catalog output stage through this gate; `POST /api/mealplan` blanks any meal slot whose name, ingredients, or visual keywords contain a forbidden term. The catalog `rankRecipes` filter via `dietTags` still runs as defense in depth.
2. **Hard diet/allergen lines at the top of every AI prompt.** `buildRecipeGenerationPrompt` and `buildMealPlanPrompt` now begin with an explicit "HARD DIET / ALLERGEN GATE" line that lists the forbidden items so Gemini cannot bury the rule under the ~100 lines of cuisine and dish-family guidance further down.
3. **Free users are catalog-only for recipe generation.** `POST /api/generate-recipes` short-circuits non-premium / non-admin requests straight to `searchCatalogRecipes` (curated `OFFLINE_RECIPES` + `sharedOfflineRecipeCache` + per-user `offlineRecipeCache`). Free users no longer touch Gemini for recipe generation, so they see canonical dish families (hawawshi, koshary, mujadara…) instead of the "chicken + onion + tomato" stub pattern observed in production. Image generation, scanning, and the 3-trial weekly meal plan still consume Gemini.
4. **Near-duplicate AI rejection.** A new pass strips stop-words / prepositions from the recipe name, sorts the remaining tokens, and rejects duplicate fingerprints inside a single AI response. Catches the "X with Y" vs "Y with X" word-swap and "Egyptian X" / "Baladi X" duplicates that the existing variety enforcer let through.
5. **Any-cuisine diversity cap.** When `preferredCuisine === "Any"`, no single cuisine is allowed to occupy more than 60% of the returned recipe set. Trims the "all 10 cards Turkish" drift seen in production.
6. **Image dedupe gate now applies to every tier.** The recipe-photo route previously short-circuited the `isRecipePhotoRecentlyUsedForDifferentSignature` check when `accessCheck.allowed` was false (credit-exhausted free users), which let the same image appear under multiple recipes. The short-circuit is removed; dedupe is uniformly active.
7. **Auto-onboarded entitlements.** `getRequestAccess` now writes a `tier: "free"` stub to `entitlements/{uid}` on first authenticated request when one is missing. Every user is now auditable from a single collection without changing the free/premium logic.
8. **Shared-pool cache diet guard.** `persistGeneratedRecipeCache` now accepts a `dietContext` and filters only the global `sharedOfflineRecipeCache` writes against diet/allergen rules. The user's private cache remains intact, but diet-violating generated recipes no longer pollute the shared pool for later free-tier/catalog users. `/api/generate-recipes`, `/api/mealplan`, `/api/mealplan/cache`, and saved-data hydration all pass or load diet context before shared writes.
9. **Shared-pool diet metadata merge.** Shared generated recipe docs now merge compatible `dietTags` with Firestore `arrayUnion`, so duplicate shared-cache writes accumulate verified diet compatibility instead of overwriting it.
10. **Legacy route alignment.** `POST /api/recipes` is still deprecated in favor of `POST /api/generate-recipes`, but its returned recipe arrays now pass through `enforceAuthenticCuisineRecipeSet` before delivery. `POST /api/analyze-image` is explicitly marked as deprecated in favor of the scan routes.

Operational helpers shipped alongside:

- `scripts/inspect-user-recipes.ts` (+ `npm run inspect:user-recipes -- <email-or-uid>`): read-only diagnostic that dumps profile, settings, entitlement, current weekly plan, recent history, and per-recipe diet/allergen analysis to a gitignored `.generated/user-inspection-<uid>-<ts>.json` file. Use to triage user reports without exposing the data in stdout.
- `.generated/` added to `apps/nutrimoment/.gitignore` so diagnostic reports cannot be accidentally committed.

Verification: lint + production build clean.
...
| Legacy/overlapping routes | **Watch** | `recipes` and `analyze-image` still exist alongside the live `generate-recipes` and `scan`. Both are marked compatibility routes; `recipes` has the cuisine post-check. Remove after clients are confirmed migrated. See weak points #6. |
...
| 6 | Legacy / overlapping API routes still on disk | Medium | `src/app/api/recipes/route.ts` and `src/app/api/analyze-image/route.ts` duplicate the live `generate-recipes` and `scan` flows. | **Mitigated (2026-05-14):** empty `src/app/api/debug/` directory removed earlier. Legacy `recipes` now applies the cuisine authenticity post-check before delivery; `analyze-image` is marked deprecated and remains ingredient-extraction only. Keep both for 1-2 weeks while verifying no clients call them, then consolidate/remove in favor of `generate-recipes` and `scan`/`scan/process`. |
