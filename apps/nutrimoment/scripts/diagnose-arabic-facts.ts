...
async function main() {
  ...
  if (cuisine === "catalog") {
    for (const name of ["water", "salmon", "rice", "mushrooms", "فطر", "200 g mushrooms", "٢٠٠ غرام فطر"]) console.log(JSON.stringify({ name, food: findArabicFood(name), normalized: await normalizeArabicInputs([name]) }));
  ...
  for (const candidate of result.recipes) {
    const checked = await buildArabicFactsEntry(candidate.facts, restrictions, candidate.source, candidate.labelReceipt, candidate.safetyReceipt);
