...
const mealTypes: RecipeMealType[] = ["lunch", "dinner"];

const profiles: HealthProfileCase[] = [
  { id: "balanced", label: "Balanced baseline", diets: [], conditions: [], allergens: [] },
  { id: "diabetes-low-sodium", label: "Diabetes + high blood pressure", diets: [], conditions: ["diabetes", "highBloodPressure"], allergens: [] },
  { id: "vegetarian-heart", label: "Vegetarian + cholesterol", diets: ["vegetarian"], conditions: ["cholesterol"], allergens: [] },
  { id: "vegan-weight-loss", label: "Vegan + weight loss + peanut allergy", diets: ["vegan"], conditions: ["weightLoss"], allergens: ["peanut"] }
];
    ...
    for (const cuisine of cuisines) {
      for (const profile of profiles) {
        for (const mealType of mealTypes) {
          index += 1;
          const result = await runCase(index, account, cuisine, profile, mealType);
          results.push(result);
          console.log(
            `${String(index).padStart(3, "0")} ${account.tier} ${cuisine} ${profile.id} ${mealType}: ` +
...
async function runCase(
  ...
  const gaps = analyzeResultGaps(recipes, cuisine, profile, account.tier);
  const dietViolation = topRecipe ? findRecipeDietViolation(topRecipe, { diets: profile.diets, allergens: profile.allergens }) : null;
  const healthViolation = topRecipe ? findRecipeHealthViolation(topRecipe, profile.conditions) : null;
  ...
  return {
    ...
    healthViolation: healthViolation ? `${healthViolation.condition}:${healthViolation.match}` : undefined,
    gaps
  };
}

function analyzeResultGaps(recipes: Recipe[], cuisine: string, profile: HealthProfileCase, tier: Tier) {
  const gaps: string[] = [];
  ...
  recipes.forEach((recipe, recipeIndex) => {
    ...
    const dietViolation = findRecipeDietViolation(recipe, { diets: profile.diets, allergens: profile.allergens });
    if (dietViolation) {
      gaps.push(`${label}: diet/allergen violation ${JSON.stringify(dietViolation)}.`);
    }

    const healthViolation = findRecipeHealthViolation(recipe, profile.conditions);
    if (healthViolation) {
      gaps.push(`${label}: health violation ${JSON.stringify(healthViolation)}.`);
    }
...
async function writeReports(results: TestCaseResult[], durationMs: number) {
  ...
  const byProfile = summarize(results, (item) => item.profile);
  const allGaps = results.flatMap((item) => item.gaps.map((gap) => ({ ...item, gap })));
  const topGapThemes = summarizeGapThemes(allGaps.map((item) => item.gap));
  ...
  await writeFile(path.join(outputDir, "comprehensive-account-matrix-report.json"), JSON.stringify(report, null, 2));
  await writeFile(
    path.join(outputDir, "comprehensive-account-matrix-report.md"),
    [
      "# NutriMoment Comprehensive Account Matrix",
      "",
      ...
      ...results
        ...
        .map((item) => [
          `### ${item.id} ${item.tier} ${item.cuisine} ${item.mealType} ${item.profile}`,
  ...
  console.log(
    `Comprehensive matrix: ${report.totalCases} cases, ${report.passLikeCases} clean, ` +
      `${report.gapCaseCount} with gaps, avg step score ${report.averageStepScore}/100`
  );
  console.log(`Report: ${path.join(outputDir, "comprehensive-account-matrix-report.md")}`);
}
