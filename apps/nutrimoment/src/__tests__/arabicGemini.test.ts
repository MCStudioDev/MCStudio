...
describe("Arabic Gemini orchestration", () => {
  ...
  it("bounds transport attempts and preserves stable JSON keys", async () => {
    await generateArabicRecipes({ ingredients: ["rice"], restrictions: { diets: [], allergens: [], conditions: [] }, count: 1, cuisine: "Any", calorieTarget: 1650, missingLimit: 2 }, Date.now() + 15_000, "r");
  ...
  it("renders structured quantities without inventing or converting measures", async () => {
    ...
    const result = await generateArabicRecipes({ ingredients: ["rice"], restrictions: { diets: [], allergens: [], conditions: [] }, count: 1, cuisine: "Any", calorieTarget: 1650, missingLimit: 2 }, Date.now() + 15_000, "r");
  ...
  it("preserves valid pairs for partial results when another ingredient is malformed", async () => {
    ...
    const result = await generateArabicRecipes({ ingredients: ["rice"], restrictions: { diets: [], allergens: [], conditions: [] }, count: 2, cuisine: "Any", calorieTarget: 1650, missingLimit: 2 }, Date.now() + 15_000, "r") as { recipes: Array<{ canonical: { ingredients: unknown[] } }> };
  ...
  it("rejects malformed JSON and clears failed translation locks", async () => {
    ...
    await expect(translateArabicSource("bad", canonical, Date.now() + 15_000, "r")).rejects.toThrow();
    await expect(translateArabicSource("bad", canonical, Date.now() + 15_000, "r")).resolves.toEqual({ recipe: {} });
  });
});
