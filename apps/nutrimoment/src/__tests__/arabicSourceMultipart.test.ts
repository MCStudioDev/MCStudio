...
describe("Arabic recipes with separately prepared components", () => {
  it("retains the separate cooking and assembly steps of koshary", async () => {
    ...
    const facts: ArabicRecipeFacts = { version: "ar-facts-v1", name: "كشري مصري", cuisine: "Egyptian", dishFamily: "Classic Egyptian Koshary", mealTypes: ["lunch", "dinner"], servings: 1,
      ingredients: ["rice", "lentils", "pasta", "chickpeas", "tomato sauce", "water"].map(name => ({ foodId: food(name), quantity: name === "water" ? 4 : 0.25, unit: "cup" as const, state: name === "chickpeas" ? "cooked" as const : "raw" as const })),
    ...
    const restrictions = { diets: ["vegan"], conditions: [], allergens: [] };
    const checked = await buildArabicFactsEntry(facts, restrictions, undefined, undefined, arabicSafetyFingerprint(facts, restrictions));
