...
import { applyArabicRateLimit as applyRateLimit } from "@/services/arabic/rateLimit";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    phase = "rate_limit";
    const limit = applyRateLimit({ uid: access.uid, feature: "recipe_photo", isPremium: access.isPremium, bypass: access.isAdmin });
    ...
    const knownReason = /^(?:ARABIC_[A-Z_]+|Arabic recipe unavailable|Arabic recipe validation failed|English source unavailable or changed|Source changed|Image generation unavailable|Invalid generated image|Unexpected image provider URL|Image too large)$/;
    logger.warn("Arabic photo request failed", { phase, errorType: error instanceof Error ? error.name : "unknown", reason: knownReason.test(code) ? code : "image_service_error" });
    if (code === "ARABIC_IMAGE_PENDING") return Response.json({ code, error: "يجري إعداد صورة الوصفة الآن.", retryAfterSeconds: 4 }, { status: 202 });
